...
struct KdTree
{
	...
	void searchHelper(Node* node, std::vector<float> target, float distanceTol, uint depth, std::vector<int>& ids){
		...
		if (node!=NULL){
			...
			if (abs(target[0] - node->point[0])<distanceTol && abs(target[1] - node->point[1])<distanceTol){
				// if box check true -> circle check (distance calculation)
				float d;
				d = sqrt((target[0]-node->point[0])*(target[0]-node->point[0]) + (target[1]-node->point[1])*(target[1]-node->point[1]));
				if (d<=distanceTol){
